...
class Progress:
    def __init__(self, total: int):
        _dpi_aware()
        self.root = tk.Tk()
        self.root.title(f"{APP_NAME} — 정리 중")
...
def review_tasks(items: List[dict], auto_threshold: float = 0.85) -> Optional[List[dict]]:
    ...
    _dpi_aware()
    root = tk.Tk()
    root.title(f"{APP_NAME} — 저장할 일정 확인")
    root.configure(bg=PAGE)
    f = _fonts()
    out = {"v": None}
    today = date.today()
...
# 결과 요약
# --------------------------------------------------------------------------
def show_summary(lines: List[str], title: str = None,
...
def show_error(msg: str) -> None:
    ...
    r = tk.Tk()
    r.withdraw()
    messagebox.showerror(f"{APP_NAME} — 오류", msg)
    r.destroy()
