...
class Wizard:
    ...
    def s0_brity(self):
        ...
        # 안내문 파일은 아무도 안 읽는다. 꼭 알아야 할 것은 첫 화면에 싣는다.
        self._p("이 넷만 아시면 됩니다.", font="bold", pady=(10, 4))
        self._box(chr(10).join((
            "· 브리티 메신저는 평소처럼 늘 켜 두세요.",
            "   꺼져 있으면 읽을 쪽지가 없어 아무것도 정리되지 않습니다.",
            "· 자리를 비우신 사이에만 정리합니다. 그래서 설치 직후에는",
            "   목록이 비어 보입니다 — 패널 아래 '⟳ 지금 확인'을 눌러 보세요.",
            "· 할 일은 체크한 것만 사라지고, 두 번 누르면 원래 쪽지가 열립니다.",
            "· 쪽지 내용은 이 컴퓨터 밖으로 나가지 않습니다.")),
            bg="#f0f9ff", fg="#0c4a6e")
